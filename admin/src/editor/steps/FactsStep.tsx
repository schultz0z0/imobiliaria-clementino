...
export const FactsStep = () => {
  ...
  return <fieldset className="wizard-fieldset"><legend>Dados principais do imóvel</legend>
    ...
    <div className="form-grid two-columns"><label className="field-group">Número de andares<input type="number" min="1" {...register('facts.floors', numberOptions)} /></label><label className="field-group">Posição do apartamento<select {...register('facts.position', { setValueAs: (value) => String(value ?? '').trim() || null })}><option value="">Selecione</option><option value="front">Frente</option><option value="back">Fundos</option><option value="side">Lateral</option><option value="middle">Meio</option></select></label></div>
  </fieldset>;
};
